...
void setup() {
  ...
  PImage friends = loadImage("Bucki.jpg");
...
background(friends);
  ...
  mustache = loadImage("Kewl.png");
  hat = loadImage("Tophat.png"); // 5. Change this to match your file name.
}

void draw() {
  // 8. Only draw the mustache when the mouse is pressed.
if (mouseButton == LEFT){
image(mustache, mouseX-150, mouseY-125);}
if (mouseButton == RIGHT) {
  image(hat, mouseX-210, mouseY-420);
}
  ...
  // 10. Draw a hat on your friends when the right mouse button is clicked: if (mouseButton == RIGHT)
}
